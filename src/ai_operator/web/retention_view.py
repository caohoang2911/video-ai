...
def retention_panel(video_id: int, duration_sec: int | None) -> dict | None:
    ...
    Returns None (panel hidden) if the retention schema is absent or a read fails, so an
    un-migrated/partial DB degrades gracefully instead of 500-ing the whole detail page."""
    try:
